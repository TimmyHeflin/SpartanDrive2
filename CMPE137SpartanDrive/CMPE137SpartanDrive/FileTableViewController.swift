...
class FileTableViewController: UITableViewController {
    ...
    @IBAction func goPostImage(_ sender: Any) {
        self.performSegue(withIdentifier: "postimage", sender: self)
    }
    
    private func GoogleSignIn_references()
    {
        let databaseref = FIRDatabase.database().reference()
        let userDatabaseRef = databaseref.child("Users")
        let email = FIRAuth.auth()?.currentUser?.email
        userDatabaseRef.setValue(["username" : email])
    }
